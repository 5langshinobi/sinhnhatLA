...
import { z } from "zod";
import { FaBirthdayCake, FaCalendarAlt, FaClock, FaFire, FaGlassCheers, FaSeedling, FaUser } from "react-icons/fa";
        ...
        <div className="bg-white rounded-xl shadow-xl overflow-hidden flex flex-col md:flex-row">
              ...
              <div className="p-4">
                ...
                <div className="flex items-center mt-3 text-sm text-gray-500">
                  <span className="flex items-center gap-1">
                    <FaUser className="inline" /> Kim Ngưu
                  </span>
                  <span className="mx-2">|</span>
                  <span className="flex items-center gap-1">
                    <FaCalendarAlt className="inline" /> 01.05.2024
                  </span>
                </div>
                <div className="flex gap-2 mt-4">
                  ...
                  <div className="w-8 h-8 rounded-full flex items-center justify-center bg-yellow-100 text-yellow-500">
                    <FaGlassCheers />
                  </div>
                  <div className="w-8 h-8 rounded-full flex items-center justify-center bg-red-100 text-red-500">
                    <FaFire />
                  </div>
                  <div className="w-8 h-8 rounded-full flex items-center justify-center bg-purple-100 text-purple-500">
                    <FaClock />
                  </div>
                </div>
              </div>
          ...
          <div className="w-full md:w-1/2 p-8">
            <h2 className="text-2xl font-bold text-center text-primary mb-6">
              Chào mừng đến với<br />Tiệc sinh nhật
            </h2>

            <Tabs defaultValue="login" className="w-full" onValueChange={(v) => setActiveTab(v as "login" | "register")}>
              <TabsList className="w-full mb-6 border-b grid grid-cols-2">
                <TabsTrigger value="login" className="w-full py-2 text-center data-[state=active]:border-b-2 data-[state=active]:border-primary data-[state=active]:text-primary font-medium">
                  Đăng nhập
                </TabsTrigger>
                <TabsTrigger value="register" className="w-full py-2 text-center font-medium">
                  Đăng ký
                </TabsTrigger>
              </TabsList>

              <TabsContent value="login" className="mt-0">
                {/* Biểu mẫu đăng nhập sử dụng form HTML truyền thống để nhận redirect từ server */}
                <form action="/api/login" method="post" className="space-y-4">
                  <div className="space-y-2">
                    ...
                    <input
                      ...
                      className="p-3 w-full border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary-light"
                    />
                    <p className="text-xs text-gray-500 mt-1">Gợi ý: Họ và tên của bạn (phạm nguyễn lan anh)</p>
                  </div>
                  ...
                  <button
                    type="submit"
                    className="w-full btn-gradient text-white py-6 rounded-lg font-medium transition-transform hover:shadow-lg hover:scale-[1.02] h-auto"
                  >
                    Đăng nhập
                  </button>
